...
A Vercel precisa saber exatamente o que publicar. O nosso código Front-End (Vite) está dentro da pasta `constructive-app`. Entre nela com o comando:
```bash
cd constructive-app
```

## Passo 3: Fazer o Login na Vercel
...
## Passo 4: Iniciar o Deploy
Com o login feito, e dentro da pasta `constructive-app`, rode apenas:
```bash
vercel
```

A Vercel fará algumas perguntas interativas no terminal:
1. **Set up and deploy “~/.../constructive-app”?** Digite `Y` (Sim) e aperte Enter.
...
4. **What’s your project’s name?** Aperte Enter para manter o nome sugerido (`constructive-app`) ou digite outro.
...
Aguarde o upload e a construção do projeto. Se der tudo certo, ela vai te devolver um link de "Preview" (Exemplo: `https://constructive-app-seu-usuario.vercel.app`).
...
Ainda dentro da pasta `constructive-app`, rode:
```bash
